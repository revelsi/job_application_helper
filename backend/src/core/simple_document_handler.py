    ...
    def upload_candidate_document(
        ...
        try:
            ...
            if text_length < 20:
                ...
                yield error_msg
                return SimpleDocumentUploadResult(
                    success=False, message=error_msg, error="Document content too short"
                )

            # Set document type based on user intent (always candidate)
            detected_type = (
                DocumentType.CV
            )  # Default; can extend to allow user selection
            ...
            error_msg = (
                f"❌ Upload failed: {e!s}. Please try again or check file format."
            )
            self.logger.error(f"Candidate document upload failed: {e}")
            yield error_msg
            return SimpleDocumentUploadResult(
                success=False, message=error_msg, error=str(e)
            )
    ...
    def upload_job_document(
        ...
        try:
            ...
            if text_length < 100:
                ...
                yield error_msg
                return SimpleDocumentUploadResult(
                    success=False, message=error_msg, error="Job description too short"
                )
